...
無料枠は雨・波・ホワイトノイズの3種、残り4種(焚き火・カフェ・風鈴・誰かの寝息)は買い切り¥480で解放するフリーミアム構成(サブスクなし)。企画背景・市場調査・価格戦略・売り方は別途まとめた戦略メモを参照。
...
app/
  theme.ts                   配色・余白トークン
  sounds.ts                  7種類の環境音の定義(id・表示名・音源ファイル・無料/有料)
  ...
  i18n.ts                      多言語対応。日/英/西/韓/独/仏/葡の7言語分のUI文言辞書と、
                                端末言語の自動検出+手動切り替えの永続化フック(useI18n)
  components/
    ...
    LanguagePicker.tsx         ヘッダー右上の言語切り替えボタン+選択モーダル
assets/
...
`app/i18n.ts` に日本語・英語・スペイン語・韓国語・ドイツ語・フランス語・ポルトガル語、計7言語分のUI文言を用意している。`expo-localization` で端末の言語設定を検出し、対応言語ならその言語で自動表示、非対応言語なら英語にフォールバックする。ヘッダー右上の言語コードボタン(例: `JA`)から手動でも切り替え可能で、選択結果はAsyncStorageに保存され次回起動時も維持される。

凪はキャラクターのセリフを持たず環境音+タイマーのUIだけなので翻訳量が少なく、他の2アプリより多言語化のハードルが低い(海外展開はまず凪から着手する方針)。**注意**: ここでの翻訳はネイティブスピーカーによるレビューを経ていない機械的な翻訳。特にストア申請文言(`docs/store-listing.md`)は別言語版を作る場合、公開前にネイティブチェックを推奨する。
...
- 実装済み: 環境音7種(誰かが隣で眠っているような「誰かの寝息」を含む)の同時ミックス再生、音量調整、集中タイマー(25/50分+カスタム)、就寝タイマー(15/30/45/60分+カスタム、終了前フェードアウト)、バックグラウンド再生設定、買い切り解放の状態管理とペイウォールUI、ブランドアイコン/スプラッシュ、7言語対応(日/英/西/韓/独/仏/葡、自動検出+手動切り替え)
- 未実装(v1では意図的に見送り): アカウント/クラウド同期、Apple Watch対応、AI機能、実際のストア決済連携、翻訳のネイティブレビュー、言語ごとの現地通貨表示(現状は全言語共通で¥表示)
