...
**追記(600mとの比較)：** その後、「北海道の規則では600mも一応の目安になり得る」との情報提供があったため500mに加えて600mでも試算した(1.5節)。木野温泉については対象地南端から約34mの区間が600m円内に入り、あわせて座標を試算した帯広徳洲会病院(未確認、1.4節)については最接近点が約604.5mと600mまで残りわずか約4.5mだった。この4.5mの差は本レポートの推定誤差(120〜200m規模)に比べて無視できるほど小さく、**600mが適用される場合、病院候補については「入るかどうか事実上判定不能」というのが実態に近い。**
...
| 帯広徳洲会病院(木野西通14丁目2-1、温泉の有無は未確認) | 42.963291 | 143.202452 | 木野西通十四丁目 代表点 |
...
| 帯広徳洲会病院(温泉の有無は未確認) | 665.5 m | **597.0 m** | 604.6 m |

計算スクリプト：[`distance_calc.py`](./distance_calc.py)、生データ：[`results.json`](./results.json)

### 1.4 追加確認事項：帯広徳洲会病院(木野西通14丁目)

利用者より「音更徳洲会病院(帯広徳洲会病院)も温泉のようだ」との情報提供があり、同院(音更町木野西通14丁目2-1)の座標を追加して試算した。2026年9月時点で公開されているウェブ情報(病院公式サイト・各種医療機関検索サイト等)からは、同院が温泉法上の源泉(温泉利用許可)を保有しているかどうかは確認できなかった。十勝地方はモール泉の賦存地域であり、病院・介護施設が独自に源泉を掘削している例も一般に見られるため、可能性自体は否定できない。

仮に同院が温泉法上の源泉に該当する場合、試算では**中心点まで約597m、北端まで約605mと木野温泉よりも規制ラインまでの余裕が小さい**(木野温泉:+97〜+377m、病院候補:+97〜+165m)。木野温泉と同様に街区代表点ベースの概算であり実際の建物・井戸座標ではない点は変わらないため、**この候補は実在・座標とも十勝総合振興局での確認を最優先とすべき対象**である。
...
利用者より「北海道の規則では600mも一応の目安になり得る」との情報提供を受け、500mに加えて600mでも同様の試算を行った。対象地の南端→北端を結ぶ代表線が、各円のどの区間と交わるかを`distance_calc.py`の`boundary_crossings()`関数で算出している(平面直角近似、300m規模の範囲では十分な精度)。

| 起点 | 500m円 | 600m円 |
|---|---|---|
| 健康ハウス木野温泉 | 対象地は全区間 圏外(最短565.5m) | **南端から約34.0mの区間が圏内**(代表線全長312mの約11%) |
| 帯広徳洲会病院(未確認) | 対象地は全区間 圏外(最短597.0m) | 代表線上は辛うじて圏外(**最接近点 約604.5m、600mまで残り約4.5m**) |

**病院候補は「実質的に判定不能」な際どさ：** 病院候補について、対象地の南端→北端を結ぶ代表線上での最接近点は約604.5mで600m円の外側になるが、その差はわずか約4.5m。本レポートの座標はいずれも丁目単位の代表点に基づく概算で、1.3節で判明した推定誤差は120〜200m規模に達する。この誤差の大きさに対して4.5mの余裕は無いに等しく、**600mが適用される場合、この候補については「入るかどうか判定できない」と捉えるのが実態に近い。** 木野温泉側の南端34m区間についても同様に、実測確認前の参考値として扱うこと。

なお、「600mも一応の目安になり得る」という情報が北海道温泉保護対策要綱の本文上の規定なのか、運用上の慣行なのかは未確認であり、十勝総合振興局への確認が必要(3.1節参照)。

---

## 2. 500m・600m規制円と敷地形状の位置関係図
...
- 赤の実線円：健康ハウス木野温泉を中心とした半径500m(北海道温泉保護対策要綱の一般的な離隔距離の推定値)
- 赤の淡い点線円：同500mの外側の半径600m(北海道内の運用で目安になり得るとの情報を踏まえた参考円)
- 紫の点線円：帯広徳洲会病院を中心と仮定した半径500m(温泉の有無は未確認の候補)、その外側の淡い点線が600m
- 青の帯：対象地の概形(面積等価モデル)、橙色部分：木野温泉の600m円に入る区間(南端から約34m)
- 図が示す通り、500m円ではいずれも対象地の外側だが、**600m円では木野温泉側の南端付近が圏内に入り、病院候補側も約4.5mの差まで接近する**
...
1. **要綱の正式名称・最新版の確認**：「北海道温泉保護対策要綱」の現行版の入手（改正の有無、離隔距離の原則値が本当に「500m」か、地域や源泉の湧出量・温度によって基準が変動しないか）。あわせて「600mも一応の目安になり得る」という情報が要綱本文の規定か運用上の慣行かを確認する。
...
9. **[新規・最優先] 帯広徳洲会病院(木野西通14丁目)の温泉法上の位置づけの確認**：同院が温泉法上の源泉を保有しているか、保有している場合はその掘削位置座標を振興局の温泉台帳で確認する。試算では中心点まで約597mと木野温泉より余裕が小さいため、実在が確認された場合は最優先で座標を特定する。

### 3.3 掘削計画・技術面での確認事項

10. **掘削位置と揚湯試験の設計**：敷地北端で掘削する場合の井戸位置候補を複数(2〜3案)用意し、それぞれが500m規制円の外側にあることを座標で明示した申請図面を準備する。
11. **既存源泉への影響評価(水理地質調査)の要否**：500m圏外であっても、近接性から水量・水位への影響評価(揚湯試験、周辺源泉のモニタリング)を求められる可能性があるか確認する。
12. **引湯配管の距離・法規制**：北端で掘削した源泉を敷地内配管で南側の温浴棟まで引湯する構想について、
    - 温泉法上、引湯管路の長さ・保温・温度低下に関する技術基準の有無
    - 音更町の宅地造成・開発行為許可における配管埋設(道路占用・自敷地内埋設)の扱い
    - 分割貸地(残地の店舗区画)を配管が横断する場合の地役権設定の要否
13. **掘削深度・地質リスク**：計画地が「旧河川敷地」「原野」との記載があり、地盤・砂礫層の状況によっては掘削深度やケーシング工法、蛇篭撤去等の造成費用に影響する可能性があるため、ボーリング業者に近隣既存源泉の井戸台帳(深度・湧出温度・成分)を踏まえた事前見積りを依頼する。

### 3.4 スケジュール・許認可プロセス

14. **温泉法に基づく掘削許可の標準処理期間**：申請から許可までの標準処理期間、現地調査の有無、周辺源泉権利者への意見照会(温泉法第4条の利害関係者協議)の有無とその期間。
15. **既存源泉権利者(木野温泉・病院候補が確認された場合は同院)との事前協議の要否**：制度上必須でなくても、500m近接案件は近隣源泉所有者への事前説明が许可審査上重要視されるケースがあるため、任意協議の要否を振興局に確認する。
