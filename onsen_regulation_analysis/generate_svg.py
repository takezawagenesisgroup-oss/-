...
X_MIN, X_MAX = -1040.0, 820.0
Y_MIN, Y_MAX = -550.0, 1500.0
...
svg = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {VIEW_W} {VIEW_H}" font-family="'IPAGothic','Noto Sans JP',sans-serif">
  ...
  <text x="{VIEW_W/2}" y="30" text-anchor="middle" font-size="18" font-weight="bold" fill="#1a202c">
    木野温泉・病院候補の500m円と対象地(木野大通東13丁目)の位置関係(概算)
  </text>
  ...
  <text x="{hospital_px[0]-radius_px*0.9:.1f}" y="{hospital_px[1]-radius_px*0.55:.1f}" class="lbl-purple">
    500m円(未確認・候補)
  </text>
  ...
  <text x="20" y="{VIEW_H-58}" class="lbl-small">凡例: 赤破線円=健康ハウス木野温泉から半径500m(北海道温泉保護対策要綱の推定離隔距離)</text>
  <text x="20" y="{VIEW_H-40}" class="lbl-small">     紫点線円=帯広徳洲会病院を中心と仮定した半径500m(温泉法上の源泉の有無は未確認)</text>
